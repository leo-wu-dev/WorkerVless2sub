...
let mytoken= ['ww'];//快速订阅访问入口, 留空则不启动快速订阅
		...
		*/
		...
		if (!userAgent.includes('subconverter') && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
			...
			const responseBody = uniqueAddresses.map(address => {
				...
				if (协议类型 == 'Trojan'){
					const trojanLink = `trojan://${uuid}@${address}:${port}?security=tls&${"SNI".toLocaleLowerCase()}=${sni}&alpn=http%2F1.1&fp=randomized&type=${type}&host=${伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(addressid + 节点备注)}`;
					return trojanLink;
				} else {
...
function surge(content, url) {
	...
	for (let x of 每行内容) {
		if (x.includes('= trojan,')) {
			const host = x.split("SNI".toLocaleLowerCase()+"=")[1].split(",")[0];
			const 备改内容 = `skip-cert-verify=true, tfo=false, udp-relay=false`;
			const 正确内容 = `skip-cert-verify=true, ${"WS".toLocaleLowerCase()}=true, ${"WS".toLocaleLowerCase()}-path=/?ed=2560, ws-headers=Host:"${host}", tfo=false, udp-relay=false`;
